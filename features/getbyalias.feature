Feature: Get by alias
  ...
  Background: dataClay deployment
    Given "User A" has a configuration file "resources/common/cfgfiles/client.properties" to be used to connect to dataClay
      ...
      And "User A" deploys dataClay with docker-compose.yml file "resources/get-by-alias/docker-compose.yml"
  ...
  Scenario: run simple get by alias
    Given "User A" starts a new session
      And "User A" runs make persistent for an object with alias "myobj"
     Then "User A" gets the object with alias "myobj"
